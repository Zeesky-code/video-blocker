...
class OptionsManager {
    ...
    setupSettingsListeners() {
        ...
        document.getElementById('restore-file').addEventListener('change', (e) => this.restoreData(e));
    ...
    setupBlockedVideosListeners() {
        ...
        // Actions
        document.getElementById('export-btn').addEventListener('click', () => this.exportBlockedList());
    ...
    calculateStats() {
        ...
        this.stats.dailyBlocked = this.blockedHashes.filter(item =>
            now - (item.added || 0) < oneDayMs
        ).length;
        this.stats.weeklyBlocked = this.blockedHashes.filter(item =>
            now - (item.added || 0) < oneWeekMs
        ).length;
    ...
    updateStats() {
        ...
        document.getElementById('weekly-blocked').textContent = this.stats.weeklyBlocked;
        document.getElementById('daily-blocked').textContent = this.stats.dailyBlocked;
        document.getElementById('storage-size').textContent = `${this.stats.storageSize} KB`;
    ...
    applyFilters() {
        const searchTerm = document.getElementById('hash-search').value.toLowerCase();
        ...
        switch (dateFilter) {
            case 'today':
                filtered = filtered.filter(item => now - (item.added || 0) < oneDayMs);
                break;
            case 'week':
                filtered = filtered.filter(item => now - (item.added || 0) < oneWeekMs);
                break;
            case 'month':
                filtered = filtered.filter(item => now - (item.added || 0) < oneMonthMs);
                break;
        }
    ...
    renderBlockedList() {
        ...
        const totalPages = Math.ceil(this.filteredHashes.length / this.itemsPerPage);
        const startIndex = (this.currentPage - 1) * this.itemsPerPage;
        const endIndex = Math.min(startIndex + this.itemsPerPage, this.filteredHashes.length);
        ...
        if (totalPages > 1) {
            ...
            document.getElementById('page-info').textContent = `Sayfa ${this.currentPage} / ${totalPages}`;
    ...
    async removeHash(hash) {
        try {
            this.blockedHashes = this.blockedHashes.filter(item => item.hash !== hash);
            await this.setStorageData(STORAGE_KEYS.BLOCKED_HASHES, this.blockedHashes);
